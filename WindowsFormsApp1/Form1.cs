...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            {
                new Bitmap(ImageSize, ImageSize),
                new Bitmap(ImageSize, ImageSize),
                new Bitmap(ImageSize, ImageSize)
            };
        ...
        private void LoadImageFromBin(string filename)
        {
            try
            {
                using (BinaryReader binaryReader = new BinaryReader(File.Open(filename, FileMode.Open)))
                {
                    GridSize = binaryReader.ReadInt32();
                    ...
                    {
                        int layerIndex = binaryReader.ReadInt32();
                        int gridX = binaryReader.ReadInt32();
                        int gridY = binaryReader.ReadInt32();
                        int tilePixelX = binaryReader.ReadInt32();
                        int tilePixelY = binaryReader.ReadInt32();
                        int red = binaryReader.ReadInt32();
                        int green = binaryReader.ReadInt32();
                        int blue = binaryReader.ReadInt32();
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
           OpenFileDialog openFileDialog = new OpenFileDialog();
            openFileDialog.Filter = "CSV файлы (*.csv)|*.csv|BIN файлы (*.bin)|*.bin|Изображения (*.jpg;*.jpeg;*.png;*.gif;*.bmp)|*.jpg;*.jpeg;*.png;*.gif;*.bmp";
            openFileDialog.InitialDirectory = "C:\\2996\\Боголюбов\\WindowsFormsApp1\\WindowsFormsApp1\\bin\\Debug";
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    LoadImageFromCsv(filename);
                }
                else if (fileExtension == ".bin")
                {
                    LoadImageFromBin(filename);
                }
                else
                {
        ...
        private void saveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            saveFileDialog.Filter = "CSV файлы (*.csv)|*.csv|Бинарные файлы (*.bin)|*.bin";
            saveFileDialog.InitialDirectory = "C:\\Users\\student\\Desktop\\WindowsFormsApp1\\WindowsFormsApp1\\project";
                            ...
                            for (int y = 0; y < GridSize; y++)
                            {
                                for (int x = 0; x < GridSize; x++)
                                {
                                    ...
                                    if (layerIndices.Count > 0)
                                    {
                                        for (int i = 0; i < layerIndices.Count; i++)
                                        {
                                            binaryWriter.Write(layerIndices[i]);
                                            binaryWriter.Write(x);
                                            binaryWriter.Write(y);
                                            binaryWriter.Write(tilePixelXValues[i]);
                                            binaryWriter.Write(tilePixelYValues[i]);
                                            binaryWriter.Write(redValues[i]);
                                            binaryWriter.Write(greenValues[i]);
                                            binaryWriter.Write(blueValues[i]);
                                        }
                                    }
                                    else
                                    {
